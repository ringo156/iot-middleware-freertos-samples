...
#define mbedtlsHighLevelCodeOrDefault(mbedTlsCode) \
    (mbedtls_high_level_strerr(mbedTlsCode) != NULL) ? mbedtls_high_level_strerr(mbedTlsCode) : pcNoHighLevelMbedTlsCodeStr
...
#define mbedtlsLowLevelCodeOrDefault(mbedTlsCode) \
    (mbedtls_low_level_strerr(mbedTlsCode) != NULL) ? mbedtls_low_level_strerr(mbedTlsCode) : pcNoLowLevelMbedTlsCodeStr
...
static TlsTransportStatus_t tlsHandshake(NetworkContext_t *pxNetworkContext,
                                         const NetworkCredentials_t *pxNetworkCredentials, const char *pcHostName);
...
static void sslContextInit(MbedSSLContext_t *pxSslContext)
{
    ...
    mbedtls_net_init(&(pxSslContext->server_fd));
}
/*-----------------------------------------------------------*/
...
{
    configASSERT(pxSslContext != NULL);
    mbedtls_net_free(&(pxSslContext->server_fd));
                         ...
                         size_t xRootCaSize)
{
    ...
    if (lMbedtlsError != 0)
    {
        ESP_LOGE(TAG, "Failed to parse server root CA certificate: lMbedtlsError[%d]= %s : %s.",
                                    ...
                                    size_t xClientCertSize)
{
    ...
    if (lMbedtlsError != 0)
    {
        ESP_LOGE(TAG, "Failed to parse the client certificate: lMbedtlsError[%d]= %s : %s.",
                 lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
    }
                             ...
                             size_t xPrivateKeySize)
{
    ...
    // Add ATECC608A priv Key
    ESP_LOGI(TAG, "Using ATECC608 private key from slot 0");
    lMbedtlsError = atca_mbedtls_pk_init(&(pxSslContext->privKey), 0);
    if (lMbedtlsError != 0)
    {
        ESP_LOGE(TAG, "failed !  atca_mbedtls_pk_init returned %02x", lMbedtlsError);
    }
    ...
    if (lMbedtlsError != 0)
    {
        ESP_LOGE(TAG, "Failed to parse the client key: lMbedtlsError[%d]= %s : %s.",
                 lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
    }
                                      ...
                                      const NetworkCredentials_t *pxNetworkCredentials)
{
    ...
    if (pxNetworkCredentials->ppcAlpnProtos != NULL)
    {
        ...
        if (lMbedtlsError != 0)
        {
            ESP_LOGE(TAG, "Failed to configure ALPN protocol in mbed TLS: lMbedtlsError[%d]= %s : %s.",
                     lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                     mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
        }
    }
    ...
    if (pxNetworkCredentials->xDisableSni == pdFALSE)
    {
        ...
        if (lMbedtlsError != 0)
        {
            ESP_LOGE(TAG, "Failed to set server name: lMbedtlsError[%d]= %s : %s.",
                     lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                     mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
        }
    }
...
/* Set Maximum Fragment Length if enabled. */
    ...
    if (lMbedtlsError != 0)
    {
        ESP_LOGE(TAG, "Failed to maximum fragment length extension: lMbedtlsError[%d]= %s : %s.",
                 lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
    }
                                     ...
                                     const NetworkCredentials_t *pxNetworkCredentials)
{
    ...
    if (lMbedtlsError != 0)
    {
        ESP_LOGE(TAG, "Failed to set default SSL configuration: lMbedtlsError[%d]= %s : %s.",
                 lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
                                         ...
                                         const NetworkCredentials_t *pxNetworkCredentials, const char *pcHostName)
{
    ...
    if (lMbedtlsError != 0)
    {
        ESP_LOGE(TAG, "Failed to set up mbed TLS SSL context: lMbedtlsError[%d]= %s : %s.",
                 lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
    ...
    else
    {
        ...
        if ((lMbedtlsError = mbedtls_net_connect(&(pxSSLContext->server_fd), pcHostName,
                                                 "8883", MBEDTLS_NET_PROTO_TCP)) != 0)
        {
            ESP_LOGE(TAG, "mbedtls_net_connect returned -%x", lMbedtlsError);
            xRetVal = eTLSTransportInternalError;
            return xRetVal;
        }
        ESP_LOGI(TAG, "Connected.");

        lMbedtlsError = mbedtls_net_set_block(&(pxSSLContext->server_fd));
        if (lMbedtlsError != 0)
        {
            ESP_LOGE(TAG, "failed! net_set_(non)block() returned -0x%x", -lMbedtlsError);
            // return SSL_CONNECTION_ERROR;
        }

        mbedtls_ssl_set_bio(&(pxSSLContext->context),
                            &(pxSSLContext->server_fd),
                            mbedtls_net_send,
                            mbedtls_net_recv,
                            NULL);
    }
    ...
    if (xRetVal == eTLSTransportSuccess)
    {
        ...
        if (lMbedtlsError != 0)
        {
            ESP_LOGE(TAG, "Failed to perform TLS handshake: lMbedtlsError[%d]= %s : %s.",
                     lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                     mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
        ...
        {
            ESP_LOGI(TAG, "(Network connection %p) TLS handshake successful.",
                     pxNetworkContext);
        }
    }
                                        ...
                                        mbedtls_ctr_drbg_context *pxCtrDrgbContext)
{
    ...
    if (lMbedtlsError != 0)
    {
        ESP_LOGE(TAG, "Failed to add entropy source: lMbedtlsError[%d]= %s : %s.",
                 lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
        xRetVal = eTLSTransportInternalError;
    }
    ...
    if (xRetVal == eTLSTransportSuccess)
    {
        ...
        if (lMbedtlsError != 0)
        {
            ESP_LOGE(TAG, "Failed to seed PRNG: lMbedtlsError[%d]= %s : %s.",
                     lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                     mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
            xRetVal = eTLSTransportInternalError;
        }
                                        ...
                                        uint32_t ulSendTimeoutMs)
{
    ...
    {
        ESP_LOGE(TAG, "Invalid input parameter(s): Arguments cannot be NULL. pxNetworkContext=%p, "
    ...
    {
        ESP_LOGE(TAG, "pucRootCa cannot be NULL.");
        xRetVal = eTLSTransportInvalidParameter;
    }
    else if ((pxSSLContext = pvPortMalloc(sizeof(MbedSSLContext_t))) == NULL)
    {
        ESP_LOGE(TAG, "Failed to allocate mbed ssl context memmory .");
        xRetVal = eTLSTransportInSufficientMemory;
    }
    else
    {
        ...
        if ((xRetVal = initMbedtls(&(pxSSLContext->entropyContext),
                                   &(pxSSLContext->ctrDrgbContext))) != eTLSTransportSuccess)
        {
            ESP_LOGE(TAG, "Failed to initialize Mbedtls %d.", xRetVal);
        }
        else if ((xRetVal = tlsSetup(pxNetworkContext, pcHostName,
                                     pxNetworkCredentials)) != eTLSTransportSuccess)
        {
            ESP_LOGE(TAG, "Failed to setup Mbedtls %d.", xRetVal);
        }
        else if ((xRetVal = tlsHandshake(pxNetworkContext, pxNetworkCredentials, pcHostName)) != eTLSTransportSuccess)
        {
            ESP_LOGE(TAG, "Failed to do TLS handshake %d.", xRetVal);
        }
        else
        {
...
void TLS_Socket_Disconnect(NetworkContext_t *pxNetworkContext)
{
        ...
        (pxNetworkContext->pParams->xSSLContext != NULL))
    {
            ...
            (lMbedtlsError != MBEDTLS_ERR_SSL_WANT_WRITE))
        {
            if (lMbedtlsError == 0)
            {
                ESP_LOGI(TAG, "(Network connection %p) TLS close-notify sent.",
                         pxNetworkContext);
            }
            else
            {
                ESP_LOGE(TAG, "(Network connection %p) Failed to send TLS close-notify: mbedTLSError[%d]= %s : %s.",
        ...
        else
        {
            ...
            // ESP_LOGI(TAG, "(Network connection %p) TLS close-notify sent; ",
            //          "received %s as the TLS status can be ignored for close-notify."(lMbedtlsError == MBEDTLS_ERR_SSL_WANT_READ) ? "WANT_READ" : "WANT_WRITE",
            //          pxNetworkContext);
        }
                        ...
                        size_t xBytesToRecv)
{
    ...
    if ((lMbedtlsError == MBEDTLS_ERR_SSL_TIMEOUT) ||
        ...
        (lMbedtlsError == MBEDTLS_ERR_SSL_WANT_WRITE))
    {
        ESP_LOGD(TAG, "Failed to read data. However, a read can be retried on this error. "
                      "mbedTLSError[%d]= %s : %s.",
                 lMbedtlsError,
                 mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
    ...
    {
        ESP_LOGE(TAG, "Failed to read data: mbedTLSError[%d]= %s : %s.",
                 lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
    }
    else
                        ...
                        size_t xBytesToSend)
{
    ...
    if ((lMbedtlsError == MBEDTLS_ERR_SSL_TIMEOUT) ||
        ...
        (lMbedtlsError == MBEDTLS_ERR_SSL_WANT_WRITE))
    {
        ESP_LOGD(TAG, "Failed to send data. However, send can be retried on this error. "
                      "mbedTLSError[%d]= %s : %s.",
                 lMbedtlsError,
                 mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
    ...
    {
        ESP_LOGE(TAG, "Failed to send data:  mbedTLSError[%d]= %s : %s.",
                 lMbedtlsError, mbedtlsHighLevelCodeOrDefault(lMbedtlsError),
                 mbedtlsLowLevelCodeOrDefault(lMbedtlsError));
    }
    else
